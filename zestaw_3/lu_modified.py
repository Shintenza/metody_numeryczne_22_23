...
if(__name__ == "__main__"):
    ...
    while(con):
        ...
        if(len(command.split()) == 4 and command.split()[0] == "add"):
            ...
            nicePrint(mod , "<multiplying row> = ")
            nicePrint(modInv , "<multiplying row>^-1 = ")
        elif(len(command.split()) == 3 and command.split()[0] == "swt"):
            firstRowNum = int(command.split()[1]) - 1
            secondRowNum = int(command.split()[2]) - 1
            permRows = mulRow([1.0] * a.shape[0])
            permRows[[firstRowNum, secondRowNum]] = permRows[[secondRowNum, firstRowNum]]
            permRowsInv = permRows.transpose() 
            nicePrint(permRows , "<switching rows> = ")
            nicePrint(permRowsInv, "<switching row>^-1 = ")
        elif(command == "quit"):
            con = False
        if(not permRows is None and sentPermInfo == False):
            print("\n\nA = P . D")
            nicePrint(a , description = "A = ")
            nicePrint(permRows , description = "P = ")
            d = np.matmul(permRows, a)
            b = np.copy(d)
            nicePrint(d , description = "D = ")
            sentPermInfo = True
        if(not mod is None):
            m = np.matmul(mod , m)
            mInv = np.matmul(mInv , modInv)
            b = np.matmul(mod , b)
            if (not permRows is None):
                print("\n\nM . D = B")
    ...
    if (not permRows is None):
        print("\n\nP . M^-1 . B = A")
        temp = np.matmul(permRows, mInv)
        nicePrint(np.matmul(temp, b) , description = "P . M^-1 . B = ")
        nicePrint(a ,                   description = "A        = ")
    else:
        print("\n\nM^-1 . B = A")
